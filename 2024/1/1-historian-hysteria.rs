...
fn main() {
    // i'm thinking, split inputs into two lists then sort
    // then iterate through inputs
    ...
    let zip_iter = left.iter().zip(right.iter());

    let mut total_difference: i32 = 0;

    for (_, (left_value, right_value)) in zip_iter.enumerate() {
